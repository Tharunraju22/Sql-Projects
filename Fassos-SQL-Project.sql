...
A.Roll Metrics
B.Driver and Custome Experience
...
group by driver_id;
4.How many of each type of roll was delievered?
...
Part-2:
7.For each customer how many delievered rolls had at least 1 change and how many had no changes?
...
8.How many rolls were delievered had both exclusions and extras
with temp_customer_orders (order_id,customer_id,roll_id,not_include_items,extra_items_included,order_date) as
(
select order_id,customer_id,roll_id,
case when not_include_items is null or not_include_items=' ' then '0' else not_include_items end as new_not_include_items,
...
1.what was the average time in minutes it took for each driver to arrive at the fassoos HQ to pickup the order?
...
5.what was the average speed for each driver for each deliver and do you notice any trend from these values?
speed= distance/time
...
duration end as integer)as duration from driver_order where distance is not null)a;

6.what is the successful delivery percentage for each driver

formula:
sdp=total order sucessfully delievered/total order taken
